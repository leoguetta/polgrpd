...
\begin{definition}\label{def:abstracthom}
  ...
  category $I$ and $X$ an object of $\M$, we define the \emph{$\M$\nbd-valued
    homology of $I$ with constant coefficient $X$} as the object of $\Loc{\M}$
  \[
    \Ho_{\M}(I,X):=\LL p_{I!}\GZ{p_I^*}X=\varhocolim I (\GZ{p_I^*}X).
  \]
\end{definition}
...
\begin{paragr}
 ...
 Let us explore the functoriality of $\Ho_{\M}(I,X)$ in $I$. Let
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Now let $X$ be an object of $\M$. Then we have a morphism
  \[
    \LL p_{I!}\GZ{p_I^*}X =\LL p_{I!}\GZ{u^*p_J^*}X \simeq \LL p_{J!} \LL u_! \GZ{u^*p_J^*}X\to \LL p_{J!}\GZ{p_J^*}X,
  \]
  where the morphism on the right is the co-unit of the ajdunction
...
\begin{definition}\label{def:classicalhomology}
  ...
  the \emph{homology of $C$}, and denote by
  \[
    \Ho(C),
  \]
  as the $\Chp(\Z)$\nbd-valued homology of $C$ with constant coefficient $\cst{\Z}$
...
\begin{paragr}
  ...
  the homology of small category $C$ is
  \[
    \Ho(C)=\hoilim_{C}\cst{\Z}.
  \]
  ...
  \]
  By co-end calculus, if we take $N=\cst{\Z}$ the we have a
  natural isomorphism
  \[
    \cst{\Z} \tens{C}M \simeq \hoilim_{C}M.
  \]
  In particular, this implies that we have
  \[
    \Ho(C)\simeq \cst{\Z} \Ltens{C} \cst{\Z},
  \]
  ...
  \[
    H_n(C)\simeq \Tor{C}{n}(\cst{\Z},\cst{\Z}).
  \]
  ...
  % Concretely, in order to compute $H_n(C)$, one has to take either a
  % projective resolution $P_{\bullet} \to \cst{\Z}$ in the category of
  % right $C$\nbd-modules, or a projective resolution of $Q_{\bullet} \to
  % \Z$ in the category of left $C$\nbd-modules, and then
  % \[
  %   H_n(C,M)\simeq H_n(P_{\bullet}\tens{C}\Z) \simeq H_n(\Z\tens{C}Q_{\bullet}).
  % \]
\end{paragr}
%%% Local Variables:
%%% mode: LaTeX
